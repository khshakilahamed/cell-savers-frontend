const image1 = "https://i.ibb.co/b26269k/slider-4.jpg";
const image2 = "https://i.ibb.co/6YH29jD/slider-3.jpg";
const image3 = "https://i.ibb.co/VTD7BK2/slider-31.jpg";

export const bannerItems = [
  {
    key: 1,
    heading: "Advanced technologies handed to us",
    text: "Our aim is to offer outstanding services straight to your home or to your office to conduct repair works or troubleshoot your computer.",
    image: image1,
  },
  {
    key: 2,
    heading: "Support and repair with lowest costs",
    text: "For us, the repair work is not just about finding the fault and fixing it, it is about quality parts that are made to last long. We make sure that each of the parts used for repairing are brand new and of the highest quality.",
    image: image2,
  },
  {
    key: 3,
    heading: "Experts of recovering lost data - fast and cheap",
    text: "Are you looking for phone or any other device repair? We carry out large number of mobile repairs every month and help you fix your device as well",
    image: image3,
  },
];
