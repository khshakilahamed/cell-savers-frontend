...
const ManageBookingPage = () => {
  ...
  const [confirmBooking, { isLoading: isConfirmLoading }] =
    useConfirmBookingMutation();

  const confirmHandler = async (id: string) => {
    try {
      const res = await confirmBooking(id).unwrap();

      if (isConfirmLoading) {
        message.loading("Confirming...");
      }
      if (res && !isConfirmLoading) {
      ...
      render: function (data: any) {
        return (
          <div className="flex items-center gap-2">
            ...
            <div>
              <Link
                href={`/${role}/manage-technician/${data?.id}/details`}
                target="blank"
                className="text-black font-bold"
              >
                <p>
